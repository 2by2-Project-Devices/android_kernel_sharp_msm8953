...
struct wakeup_source_log {
	char name[MAX_WAKEUP_SOURCES_NAME + 1];
...
static struct wakeup_source_log wakeup_source_logs[MAX_WAKEUP_SOURCES_COUNT];
...
static ktime_t *sh_get_wakeup_source_last_total(const char *name)
{
	int i;
	int empty = -1;

	for (i = 0; i < MAX_WAKEUP_SOURCES_COUNT; i++) {
		if (wakeup_source_logs[i].used &&
		    !strncmp(wakeup_source_logs[i].name, name,
			     MAX_WAKEUP_SOURCES_NAME))
			return &wakeup_source_logs[i].last_total_time;
		if (!wakeup_source_logs[i].used && empty < 0)
			empty = i;
	}
	...
	strlcpy(wakeup_source_logs[empty].name, name,
		sizeof(wakeup_source_logs[empty].name));
	wakeup_source_logs[empty].last_total_time = ktime_set(0, 0);
	wakeup_source_logs[empty].used = true;

	return &wakeup_source_logs[empty].last_total_time;
}
...
char *sh_write_buffer_wakeup_sources_internal(char *buffer, struct list_head *wakeup_sources)
{
	...
	list_for_each_entry_rcu(ws, wakeup_sources, entry){
		...
		last_total_time = sh_get_wakeup_source_last_total(ws->name);
		if (last_total_time) {
			delta = ktime_sub(total_time, *last_total_time);
...
static void init_data(void){
	...
	memset(wakeup_source_logs, 0, sizeof(wakeup_source_logs));
